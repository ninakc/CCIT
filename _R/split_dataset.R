...
split_dataset <- function(data, num_exposure_cats) {
  ...
  a_vals <- seq(min(data$treat), max(data$treat), length.out = num_exposure_cats)
  ...
  subsample_index <- caret::createFolds(as.factor(data$treat_level), list = TRUE, k = 3)
  data <- data %>%
    mutate(subsample = ifelse(row_number() %in% subsample_index[[2]], 'exploration',
                              ifelse(row_number() %in% subsample_index[[3]], 'validation', 'inference')))
